...
public:
   ...
   void operator() ( int x, int y, unsigned char value );
...
inline unsigned char &  GrayScaleImage::getElement ( int x, int y ) {
   ...
   /* Replicate pixels at the borders to handle access to out of image
    * points when using bilinear interpolation
    */
   int i, j;
   i = (x == size_[0]) ? x-1 : x;
   j = (y == size_[1]) ? y-1 : y;

   return image_[ j * size_[0] + i ];
}
